...
void FPhysicsDebugRenderPass::Render(const std::shared_ptr<FEditorViewportClient>& Viewport)
{
    ...
    physx::PxScene* PhysicsScene = GEngine->ActiveWorld->GetPhysicsScene();
    if (!PhysicsScene)
        return;
...
void FPhysicsDebugRenderPass::DrawDebugData(physx::PxScene* PhysicsScene, const std::shared_ptr<FEditorViewportClient>& Viewport)
{
    ...
    if (rb.getNbLines() > 0)
    {
        ...
        for (physx::PxU32 i = 0; i < rb.getNbLines(); i++)
        {
            ...
            LineVertices.Add({ FromPxVec3(line.pos0), FromPxColor(line.color0) });
            LineVertices.Add({ FromPxVec3(line.pos1), FromPxColor(line.color1) });
        }
    ...
    if (rb.getNbPoints() > 0)
    {
        ...
        for (physx::PxU32 i = 0; i < rb.getNbPoints(); i++)
        {
            ...
            PointVertices.Add({ FromPxVec3(point.pos), FromPxColor(point.color) });
        }
    ...
    if (rb.getNbTriangles() > 0)
    {
        ...
        for (physx::PxU32 i = 0; i < rb.getNbTriangles(); i++)
        {
            ...
            TriangleVertices.Add({ FromPxVec3(tri.pos0), FromPxColor(tri.color0) });
            TriangleVertices.Add({ FromPxVec3(tri.pos1), FromPxColor(tri.color1) });
            TriangleVertices.Add({ FromPxVec3(tri.pos2), FromPxColor(tri.color2) });
        }
